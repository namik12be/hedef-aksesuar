...
/* ---- Ana Sayfa Düzenle ----
   "Ana Sayfayı Düzenle" butonuna basınca gerçek #homeView doğrudan (kopyasız) düzenlenebilir
   hale gelir — böylece görünen her zaman sitenin birebir kendisidir. Düzenlenebilir alanlar
   (eyebrow, hero başlık/alt yazı, şerit yazıları) data-hk ile işaretli; "Çok Satan"/"Popüler"/
   "Sana Özel" başlıkları ve buton yazıları kasıtlı olarak data-hk TAŞIMAZ, dolayısıyla asla
   contenteditable olmaz. ---- */
...
function captureHomeEditorSnapshot(){
  const snap = {};
  document.querySelectorAll('#homeView [data-hk]').forEach(elx => { snap[elx.dataset.hk] = elx.innerHTML; });
  return snap;
}
function applyHomeEditorSnapshot(snap){
  document.querySelectorAll('#homeView [data-hk]').forEach(elx => {
...
function updateHomeEditorUndoRedoButtons(){
  document.getElementById('homeEditUndoBtn').disabled = homeEditorHistoryIndex <= 0;
  document.getElementById('homeEditRedoBtn').disabled = homeEditorHistoryIndex >= homeEditorHistory.length - 1;
}
...
function enterHomeEditMode(){
  ...
  document.querySelectorAll('#homeView [data-hk]').forEach(elx => elx.setAttribute('contenteditable', 'true'));
  document.getElementById('homeEditBar').style.display = 'flex';
...
function exitHomeEditMode(){
  document.querySelectorAll('#homeView [data-hk]').forEach(elx => elx.removeAttribute('contenteditable'));
  document.body.classList.remove('home-editing');
  document.getElementById('homeEditBar').style.display = 'none';
  applyHomeContentOverrides(); // kaydedilmemiş değişiklikler varsa son kaydedilen hale döner
  showAdmin();
}
document.getElementById('homeEditEnterBtn').addEventListener('click', enterHomeEditMode);
document.getElementById('homeEditExitBtn').addEventListener('click', exitHomeEditMode);

document.getElementById('homeEditUndoBtn').addEventListener('click', () => {
  if(homeEditorHistoryIndex <= 0) return;
  homeEditorHistoryIndex--;
  applyHomeEditorSnapshot(homeEditorHistory[homeEditorHistoryIndex]);
  updateHomeEditorUndoRedoButtons();
});
document.getElementById('homeEditRedoBtn').addEventListener('click', () => {
...
document.getElementById('homeView').addEventListener('input', (e) => {
...
document.getElementById('homeEditBar').addEventListener('mousedown', (e) => {
  ...
  const iconBtn = e.target.closest('.home-icon-btn');
  if(iconBtn){
    e.preventDefault();
    const svg = HOME_EDITOR_ICONS[Number(iconBtn.dataset.hicon)];
...
document.getElementById('homeEditSaveBtn').addEventListener('click', () => {
  document.querySelectorAll('#homeView [data-hk]').forEach(elx => { HOME_CONTENT[elx.dataset.hk] = elx.innerHTML; });
